...
# set -e tells the shell to immediately exit the script if any command returns a non‑zero (error) exit status.
set -e
...
# for database we need to install postgres
# --- Install PostgreSQL ---
echo "📦  Installing PostgreSQL..."
sudo apt install -y postgresql postgresql-contrib


# we need reverse proxy server so we need to install nginx along with frontend layer
# --- Install Nginx ---
echo "📦  Installing Nginx..."
sudo apt install -y nginx

# PM2 ensures that your Node.js application stays alive, restarts automatically, scales across CPUs, and runs reliably in production.
...
# 169.254.169.254 is the AWS EC2 Instance Metadata Service (IMDS) IP address.
# it is a link‑local address that exists only inside an EC2 instance.
# AWS exposes instance metadata (information about the running EC2 instance) through this endpoint.
# 169.254.169.254 is a special AWS internal IP used by EC2 to expose instance metadata such as public IP, instance ID, and IAM credentials.
